...
pl "./output/phi.txt" us 1:2 ti "$V(x)$" w l ls 1, \
 "" us 1:3 ti "" w l dt 3 lc rgb "red" lw 2, "" us 1:4 ti "first peak" w filledcur ls 2

set label 1 "(b)" left at first -4,1.5
set ylab "$|| \\varphi_{1}(x) ||^{2}$"
set title sprintf("$E_{1} = %.3f - %.6f i$", ER1, abs(EI1))
pl "" us 1:2 ti "$V(x)$" w l ls 1, \
 "" us 1:5 ti "" w l dt 3 lc rgb "blue" lw 2, "" us 1:6 ti "second peak" w filledcur ls 3
